...
function renderDashboard(){
 ...
 const last=document.getElementById('dashboard-last');if(last){const m=matches().filter(x=>x.status==='Finalizado').sort((a,b)=>b.date.localeCompare(a.date))[0];last.innerHTML=m?`<div class="match-main"><div>${badge(club(m.homeId))}<div class="club-name">${esc(m.home)}</div></div><div><div class="score">${m.homeScore} - ${m.awayScore}</div><span class="status-pill">FINALIZADO</span></div><div>${badge(club(m.awayId))}<div class="club-name">${esc(m.away)}</div></div></div>`:'<div class="empty-state">Todavía no hay partidos cargados.</div>'}
 const next=document.getElementById('dashboard-next');if(next){const rows=matches().filter(x=>x.status!=='Finalizado').sort((a,b)=>a.date.localeCompare(b.date)).slice(0,3);next.innerHTML=rows.map(m=>`<div class="fixture">${dateBox(m.date)}${badge(club(m.homeId),true)}<span class="team-text">${esc(m.home)}</span><span class="versus">VS<small>${esc(m.time)}</small></span><span class="team-text">${esc(m.away)}</span>${badge(club(m.awayId),true)}</div>`).join('')||'<div class="empty-state">No hay próximos partidos cargados.</div>'}
 const high=document.getElementById('dashboard-highlights');if(high){const ps=players();if(!ps.length){high.innerHTML='<div class="empty-state" style="grid-column:1/-1">Todavía no hay estadísticas cargadas.</div>'}else{const top=(key)=>ps.slice().sort((a,b)=>(Number(b[key])||0)-(Number(a[key])||0))[0];const g=top('goals'),e=top('eff'),a=top('assists'),s=top('sanctions');high.innerHTML=[[g,'GOLEADOR DEL TORNEO','goals','GOLES'],[e,'EFECTIVIDAD','eff','%'],[a,'MÁS ASISTENCIAS','assists','ASISTENCIAS'],[s,'MÁS SANCIONES','sanctions','SANCIONES']].map(([p,k,key,u])=>`<div class="highlight-item"><span class="highlight-kicker">${k}</span><b class="highlight-name">${esc(p.name)}</b><span class="highlight-club">${esc(p.club)}</span><strong class="highlight-number">${p[key]}${u==='%'?'%':''}</strong><span class="highlight-unit">${u}</span></div>`).join('')}}
}

function initAdminLocks(){
 document.querySelectorAll('.admin-lock').forEach(el=>el.addEventListener('click',e=>{
  e.preventDefault();
  showToast('Acceso restringido. La carga se habilitará sólo para usuarios autorizados.');
 }));
}

document.addEventListener('DOMContentLoaded',()=>{renderShell();renderDashboard();renderClubes();renderJugadores();renderPlanteles();renderPartidos();renderParticipaciones();renderStats();renderPartidoDetalle();initForms();initSettings();initReports();initAdminLocks();});

...
export function fixtureScopeKey(fixture) {
  ...
  return [
    ...
    local,
    teamCode(fixture?.local_equipo_codigo),
    visitante,
    teamCode(fixture?.visitante_equipo_codigo)
  ].join('|');
}
...
function sourceType(fixture) {
  ...
  if (!['fecha_normal', 'reprogramacion'].includes(type)) {
    throw new Error(`tipo_fuente inválido: ${type}`);
  }
  return type;
}
...
/**
 * Prioriza programaciones sin mezclar cruces de categorías/divisiones/ramas.
 *
 * Reglas:
 * - con equipo_id, la identidad deportiva es local_equipo_id + visitante_equipo_id;
 * - antes de tener IDs, el scope incluye categoría, división, rama y códigos A/B/C/D;
 * - una única reprogramación distinta gana sobre la fecha normal;
 * - dos reprogramaciones distintas del mismo cruce son ambiguas;
 * - dos fechas normales distintas del mismo cruce, sin reprogramación inequívoca, son ambiguas.
 */
export function prioritizeFixtureSchedules(fixtures) {
  ...
  fixtures.forEach((fixture, index) => {
    const scope = fixtureScopeKey(fixture);
    sourceType(fixture);
    scheduleKey(fixture);
  ...
  for (const [scope, rows] of groups) {
    ...
    if (rescheduled.length === 1) {
      const chosen = { ...rescheduled[0] };
      delete chosen.__index;
      selected.push({
        ...chosen,
        fuente_priorizada: 'reprogramacion',
        versiones_detectadas: rows.length,
        fecha_programacion_original: normal.length === 1 ? normal[0].fecha : null,
        hora_programacion_original: normal.length === 1 ? normal[0].hora ?? null : null
      });
      continue;
    }

    if (rescheduled.length > 1) {
      ambiguous.push({ scope, tipo: 'multiples_reprogramaciones', versiones: rows.map(stripInternal) });
      continue;
    }

    if (normal.length === 1) {
      const chosen = { ...normal[0] };
      delete chosen.__index;
      selected.push({
        ...chosen,
        fuente_priorizada: 'fecha_normal',
        versiones_detectadas: rows.length,
        fecha_programacion_original: null,
        hora_programacion_original: null
      });
      continue;
    }

    if (normal.length > 1) {
      ambiguous.push({ scope, tipo: 'mismo_cruce_repetido', versiones: rows.map(stripInternal) });
    }
  }

...
const workItem = {
  ...
  source: {
    ...
    page_title: 'Programación Fecha 1',
    pdf_url: sourceUrl,
...
assert.equal(result.parsed.local.goles, 20);
assert.equal(result.parsed.visitante.goles, 27);
assert.equal(result.parsed.resumen.jugadores, 32);
assert.equal(result.parsed.resumen.goles, 47);
assert.equal(result.validation.player_goal_totals_match_score, true);
assert.equal(result.validation.expected_match_checked, true);

assert.throws(
  () => parsePlanillaDryRun({ workItem: { ...workItem, method: 'POST' }, extractedText }),
  /GET/,
);
assert.throws(
  () => parsePlanillaDryRun({ workItem: { ...workItem, auth_used: true }, extractedText }),
  /autenticación/,
);
assert.throws(
  () => parsePlanillaDryRun({ workItem, extractedText, expected: { goles_visitante: 26 } }),
  /Marcador visitante inesperado/,
);
assert.throws(
  () => parsePlanillaDryRun({ workItem, extractedText: '' }),
  /vacío/,
);

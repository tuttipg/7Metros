...
La respuesta debe exponer un `ReadableStream` con `getReader()`. La lectura es incremental y aplica el límite de bytes durante la descarga: si el cuerpo supera `maxBytes`, cancela el reader y falla antes de acumular el resto del archivo en memoria. Esto cubre también respuestas sin `Content-Length`. Una respuesta sin streaming falla cerrado; no existe fallback a `arrayBuffer()` y las regresiones verifican que ese método no se invoque.
...
El blueprint no contiene nodos HTTP, Supabase, bases de datos, webhooks, persistencia ni `credentials`. Tampoco activa el workflow. Está pensado como artefacto importable/de referencia para el wiring DRY RUN; la ejecución real dentro de una instancia n8n requiere conectar la entrada binaria producida por el downloader SAFE. El 2026-09-19 se validó en la instancia n8n Cloud real del proyecto la compatibilidad runtime del patrón de descarga/binario, `Extract From File`, SHA-256, envelope/rejoin, parser, gate pre-Supabase e idempotencia, siempre sin escrituras. La evidencia y sus límites están registrados en [`docs/femebal-n8n-runtime-validation-2026-09-19.md`](./femebal-n8n-runtime-validation-2026-09-19.md).

...
function headers(range = null, extra = {}) {
  ...
  const result = { apikey: key, Authorization: `Bearer ${key}`, Accept: 'application/json', 'Content-Type': 'application/json' };
...
function summaryFromMatches(matches, clubs, players) {
  ...
  const goals = finalizados.reduce((sum, row) => sum + Number(row.goles_local || 0) + Number(row.goles_visitante || 0), 0);
  return { clubs, players, matches: matches.length, finished: finalizados.length, goals, avgGoals: finalizados.length ? Number((goals / finalizados.length).toFixed(1)) : null };
}
...
async function loadGlobalSummaryView() {
  try {
    ...
    const row = rows[0];
    if (!row) return null;
    return { clubs: Number(row.clubes || 0), players: Number(row.jugadores || 0), matches: Number(row.partidos || 0), finished: Number(row.partidos_con_resultado || 0), goals: Number(row.goles || 0), avgGoals: row.promedio_goles == null ? null : Number(row.promedio_goles) };
  } catch { return null; }
}

export function filterMatchesToTeamIds(matches, teamIds) {
  const allowed = teamIds instanceof Set ? teamIds : new Set((teamIds || []).map(Number).filter(Number.isFinite));
  return (matches || []).filter(row => {
    ...
    return Number.isFinite(homeTeamId) && Number.isFinite(awayTeamId) && allowed.has(homeTeamId) && allowed.has(awayTeamId);
  });
}
...
export async function loadPublicDataset(seasonId) {
  ...
  const equipos = await supabaseGetAll('equipos', `?select=id,club_id,temporada_id,categoria,division,rama,equipo_codigo,nombre_femebal,activo&temporada_id=eq.${season}`);

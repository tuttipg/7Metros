...
class T(unittest.TestCase):
    ...
    def test_pdf_discovery_allowlist_and_provenance(self):
        ...
        rows=discover_pdfs(PAGE,s); self.assertEqual(len(rows),2)
        self.assertTrue(all(x.pdf_url.startswith('https://femebal.com/wp-content/uploads/') for x in rows))
        self.assertTrue(_is_official_upload_pdf(rows[0].pdf_url)); self.assertFalse(_is_official_upload_pdf('https://femebal.com/documentos/reglamento.pdf'))
    ...
    def test_reject_unsafe_urls(self):
        for url in ['http://femebal.com/x','https://evil.example/x','https://user:pass@femebal.com/x','https://femebal.com:444/x']:
    ...
    def test_redirect_handler_blocks_external_destination(self):
        ...
        with self.assertRaises(ValueError): h.redirect_request(req,None,302,'Found',{},'https://evil.example/collect')
